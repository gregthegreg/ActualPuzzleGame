...
if(state=PieceStateBlue.lock)
{
	locked = true;	
}
else locked = false
if(state=PieceStateBlue.falling)
{
	falling = true;	
}
else falling = false
if(state=PieceStateBlue.shot)
{
	shot = true;	
}
else shot = false
...
#endregion
if(state == PieceStateBlue.carried)
{	#region
...
else if(state == PieceStateBlue.shot)
{
...
else if(state == PieceStateBlue.midairlock)
{
	
	
}
else if(state == PieceStateBlue.lock)
{
	...
	if(!spent)
	{
		...
		var inst
		 inst  = instance_create_layer(x,y,"Chain",oChaincounterBlue);
		inst.tired = true;
		inst.chain[0] = self;
		oGame.chain[0] = self;
...
else if(state == PieceStateBlue.falling)
{
	oGame.canfall = false
